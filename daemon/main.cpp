...
int main (int argc, char* argv[])
{
    ...
    QObject::connect(qApp, &QApplication::commitDataRequest, [](QSessionManager &sessionManager){
        if (sessionManager.allowsInteraction()) {
            deletePluginManager();
            USD_LOG(LOG_DEBUG,"allow interect");
        } else {
            USD_LOG(LOG_DEBUG,"not allow interect yet");
        }
    });
    ...
    if(translator.load(QString("/usr/share/ukui-settings-daemon/daemon/res/i18n/%1_%2").arg("ukui-settings-daemon").arg(QLocale::system().name()))) {
        app.installTranslator(&translator);
        qDebug()<<QString("/usr/share/ukui-settings-daemon/daemon/res/i18n/%1_%2").arg("ukui-settings-daemon").arg(QLocale::system().name());
    } else {
...
static void stop_daemon ()
{
    ...
    if (isStarting) {
        ...
        USD_LOG(LOG_DEBUG,"start wait");
        sleep(3);
        USD_LOG(LOG_DEBUG,"wait over");
        if (reply.isValid()) {
