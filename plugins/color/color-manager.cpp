...
{
    bool isAllDay = settings->get(COLOR_KEY_ALLDAY).toBool();
    bool isEnable = settings->get(COLOR_KEY_ENABLED).toBool();
...
void ColorManager::NightLightRecheck(ColorManager *manager)
{
    ...
    if (manager->settings->get(COLOR_KEY_AUTO_THEME).toBool()) {
        if (manager->cached_sunrise > 0.f && manager->cached_sunset > 0.f) {
            ...
            manager->settings->set(COLOR_KEY_AUTOMATIC_FROM, theme_from);
            manager->settings->set(COLOR_KEY_AUTOMATIC_TO, theme_to);
        } else {
                                    ...
                                    gpointer user_data)
{
    ...
    USD_LOG(LOG_DEBUG,"set latitude:%f  longitude:%f", latitude, longitude);
//    manager->StopGeoclue();//获取成功一次后停止服务。。
}
...
void ColorManager::SettingsChangedCb(QString key)
{
    ...
    if (key == COLOR_KEY_ALLDAY || key == COLOR_KEY_ENABLED) {
        ...
        if (darkMode && false == ret) {
            ...
            if (qt_settings->get(QT_THEME_KEY).toString() == "ukui-dark") {
                m_darkModeChangedBySelf = true;
                settings->set(COLOR_KEY_DARK_MODE,true);
                settings->apply();
            }
    ...
    if (key == COLOR_KEY_DARK_MODE) {
        ...
        if (settings->get(key).toBool()) {//进入夜间模式
            ...
            if (false == settings->get(COLOR_KEY_AUTO_THEME).toBool()) {
                   USD_LOG_SHOW_PARAMS(settings->get(COLOR_KEY_STYLE_NAME_DM).toString().toLatin1().data());
                if (settings->get(COLOR_KEY_STYLE_NAME_DM).toString() != "") {
                    ...
                    USD_LOG(LOG_DEBUG, "COLOR_KEY_STYLE_NAME_DM is nullptr");
                }
            } else {
...
bool ColorManager::ColorManagerStart()
{
    ...
    QTimer::singleShot(ms, this, [=](){
        ...
        connect(qt_settings, SIGNAL(changed(QString)) , this ,SLOT(qtSetingsChangedCb(QString)));
        connect(settings,SIGNAL(changed(QString)),this,SLOT(SettingsChangedCb(QString)));
        SettingsChangedCb(COLOR_KEY_ENABLED);
        mColorGtkConfig->connectGsettingSignal();
    });
