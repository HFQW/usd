...
//    static void onContextStreamRemoved(MateMixerContext*,char*,MediaKeysManager*);
    ...
    void doChangePerformanceMode();
    void doTouchpadAction(int);
    void doSoundAction(int);
    void doSoundActionALSA(int);
    void doMicSoundAction();
    void doBrightAction(int);
    void updateDialogForVolume(uint,bool);
    void executeCommand(const QString& program, const QStringList &arguments = QStringList());
...
private Q_SLOTS:
    ...
    void doSettingsChangeAction(QString);

    void sinkVolumeSlot(int);
    void sinkMuteSlot(bool);
...
private:
    ...
    QDBusInterface*   m_dbusControlCenter = nullptr;

    KScreen::ConfigPtr m_config;

    QTimer            *mTimer;
    QGSettings        *mSettings;
    QGSettings        *m_settingsKeyState;
    QGSettings        *pointSettings;
    QGSettings        *sessionSettings;
    QGSettings        *shotSettings;
    QGSettings        *m_pPowerSettings;
...
//    MateMixerStreamControl  *mInputControl;
    ...
    //可以通过powermanager 接口设置亮度
    bool               m_canSetBrightnessByPower = true;
