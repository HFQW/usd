...
XDevice* device_is_touchpad (XDeviceInfo *deviceinfo)
{
    ...
    if (deviceinfo->type == XInternAtom (QX11Info::display(), XI_TOUCHPAD, False)) {
        try {
    ...
    if(UsdBaseClass::isNotebook()) {
        if ((deviceinfo->type == XInternAtom(QX11Info::display(), XI_MOUSE, True)) && strstr(deviceinfo->name, "PS\/2")) {
            try {
                device = XOpenDevice (QX11Info::display(), deviceinfo->id);
                if (device == NULL) {
                    throw 1;
...
bool device_is_mouse(XDeviceInfo *deviceinfo)
{
    XDevice *device = nullptr;
    if ((deviceinfo->type == XInternAtom(QX11Info::display(), XI_MOUSE, False)) && !strstr(deviceinfo->name, "PS\/2")) {
        return true;
    }
    return false;
}
...
bool query_device_had_property(XDeviceInfo *device_info,const char *property_name)
{
    XDevice *device;
    int nProps = 0;
    Atom* type_data;
...
//    USD_LOG(LOG_DEBUG,"device name：%s",device_info->name);
    ...
    type_data =  XListDeviceProperties(display ,device, &nProps);
    Xdata = XInternAtom (QX11Info::display(), property_name, True);

    //USD_LOG_SHOW_PARAM1(nProps);
    //USD_LOG_SHOW_PARAM1(Xdata);
    //USD_LOG_SHOW_PARAMS(property_name);
...
/* This is an example of the last function.
                        ...
                        bool         enabled)
{
    ...
    try {
        ...
        if(gdk_x11_display_error_trap_pop (gdk_display_get_default()))
             USD_LOG(LOG_DEBUG,"Error while setting %s on \"%s\"", property_name, device_info->name);
...
void MouseManager::SetMotionLibinput (XDeviceInfo *device_info , bool touchpad)
{
    XDevice *device;
    Atom prop;
    Atom type;
    ...
    try {
        ...
        /* panel gives us a range of (old [1.0-10.0] ,new [10-0.1]), map to libinput's [-1, 1]
         *
        ...
        accel = (motion_acceleration - 1.0) * 2.0 / 7.0 - 1;
...
void MouseManager::SetMotionLegacyDriver (XDeviceInfo     *device_info)
{
    ...
    Display * dpy = QX11Info::display();//gdk_x11_get_default_xdisplay ();//QX11Info::display();
...
void MouseManager::SetMotion (XDeviceInfo *device_info , bool touchpad)
{
    if (query_device_had_property (device_info, "libinput Accel Speed")) {

        SetMotionLibinput (device_info ,touchpad);
    } else {
        SetMotionLegacyDriver (device_info);
    }
    if(query_device_had_property (device_info, "Device Accel Constant Deceleration")) {

        SetTouchpadMotionAccel(device_info);
    }
    if(query_device_had_property (device_info, "libinput Accel Profile Enabled")) {

        SetMouseAccel(device_info);
    }
}

void MouseManager::SetMotionAll(bool touchpad)
{
    XDeviceInfo *device_info = nullptr;
    int n_devices;
    int i;
    ...
    for (i = 0; i < n_devices; i++) {
        SetMotion (&device_info[i],touchpad);
    }
...
//    Display * display = GDK_DISPLAY_XDISPLAY(gdk_display_get_default());
    ...
    if (!prop) /* no evdev devices */
        return;
    try {
        USD_LOG(LOG_DEBUG,"ready open:%d",device_info->id);
        device = XOpenDevice (display, device_info->id);
        if (device == NULL)
            throw 1;
        rc = XGetDeviceProperty (display,
...
void MouseManager::SetMouseWheelSpeedByImwheel(int speed)
{
    if(speed <= 0)
          return;
    GPid pid;
    ...
    char **args;
    int    argc;
    QString str = "/usr/bin/imwheel -k -b \"4 5\"";
    if( g_shell_parse_argv (str.toLatin1().data(), &argc, &args, NULL)){
        g_spawn_async (g_get_home_dir (),
                       args,
                       NULL,
...
void MouseManager::SetMouseWheelSpeedByLibinput(XDeviceInfo *device_info)
{
    XDevice *device;
    Atom prop;
    Atom type;
    ...
    float_type = property_from_name ("FLOAT");
    if (!float_type) {
        USD_LOG(LOG_WARNING,"property name to atom is failed");
        return;
    }

    prop = property_from_name ("libinput Button Scrolling Speed");
    if (!prop) {
        USD_LOG(LOG_WARNING,"property name to atom is failed");
        return;
    }
    try {
        device = XOpenDevice (dpy, device_info->id);
        if (device == NULL) {
            USD_LOG(LOG_WARNING,"error : xopen device is failed");
            return;
        }
        float speed = settings_mouse->get(KEY_MOUSE_WHEEL_SPEED).toFloat();
        USD_LOG(LOG_DEBUG,"set mouse speed %f",speed);
        rc = XGetDeviceProperty (dpy,device, prop, 0, 1, False, float_type, &type,
                                 &format, &nitems, &bytes_after, &data.c);
...
void MouseManager::SetMouseWheelSpeed(XDeviceInfo *device_info)
{
    if (query_device_had_property(device_info, "libinput Button Scrolling Speed")) {
        QProcess::startDetached("imwheel -kq");
        m_findScrollSpeed = true;
        SetMouseWheelSpeedByLibinput(device_info);
    } else if(!m_findScrollSpeed) {
        SetMouseWheelSpeedByImwheel(settings_mouse->get(KEY_MOUSE_WHEEL_SPEED).toInt());
    }
}

void MouseManager::SetMouseWheelSpeedAll()
{
    XDeviceInfo *device_info = nullptr;
    int n_devices;
    device_info = XListInputDevices (gdk_x11_get_default_xdisplay (), &n_devices);
    ...
    m_findScrollSpeed = false;
    for (int i = 0; i < n_devices; i++) {
        if (device_is_mouse(&device_info[i])) {
            SetMouseWheelSpeed(&device_info[i]);
        }
    }
    if (!device_info) {
        XFreeDeviceList (device_info);
...
void MouseManager::MouseCallback (QString keys)
{
    if (keys.compare(QString::fromLocal8Bit(KEY_LEFT_HANDED))==0){
        ...
        SetMouseWheelSpeedAll();
    } else if (keys.compare(QString::fromLocal8Bit(KEY_MOUSE_NATRUAL_SCROLLING)) == 0){
        SetNaturalScrollAll(false);
    } else{
...
void mouse_set_bool (XDeviceInfo *device_info,
                     const char  *property_name,
                     int          property_index,
                     bool          enabled)
{
    XDevice *device;
    if (device_info->type != XInternAtom (QX11Info::display(), XI_MOUSE, True))
            return;
    device = XOpenDevice (QX11Info::display(), device_info->id);
    if (device == NULL) {
            return;
    }
    property_set_bool (device_info, device, property_name, property_index, enabled);
...
//     Display * display = GDK_DISPLAY_XDISPLAY(gdk_display_get_default());
    ...
    USD_LOG(LOG_DEBUG,"setting scroll method on %s", device_info->name);
    try {
        rc = XGetDeviceProperty (display, device, prop, 0, 2,
                                 False, XA_INTEGER, &type, &format, &nitems,
                                 &bytes_after, &data);
        ...
        USD_LOG(LOG_DEBUG,"Error in setting scroll method on \"%s\"", device_info->name);
    }
...
void MouseManager::setNaturalScrollSynaptics (XDeviceInfo *device_info,
                                                    bool isTouchpad)
{
    Q_UNUSED(isTouchpad)
    XDevice *device;
...
void MouseManager::setNaturalScrollLibinput (XDeviceInfo *device_info,
                                  bool       isTouchPad)
{
    ...
    if(isTouchPad) {
        ...
        bool natural_scroll = settings_mouse->get(KEY_MOUSE_NATRUAL_SCROLLING).toBool();
        mouse_set_bool (device_info, "libinput Natural Scrolling Enabled",
                           0, natural_scroll);
    }
}


void MouseManager::setNaturalScroll (XDeviceInfo *device_info,bool isTouchPad)
{
    if (property_from_name ("Synaptics Scrolling Distance")) {//是否有存在此属性的鼠标，未知
        setNaturalScrollSynaptics (device_info, isTouchPad);
    }
    if (property_from_name ("libinput Natural Scrolling Enabled")) {//触摸板和鼠标都可能有此属性
        setNaturalScrollLibinput (device_info, isTouchPad);
    }
}

void MouseManager::SetNaturalScrollAll (bool isTouchPad)
{
    int numdevices, i;
    XDeviceInfo *devicelist = XListInputDevices (QX11Info::display(), &numdevices);
    ...
    if (devicelist == NULL)
            return;
    for (i = 0; i < numdevices; i++) {
            setNaturalScroll (&devicelist[i],isTouchPad);
    }
    XFreeDeviceList (devicelist);
}
...
/* Get device node for gudev
   node like "/dev/input/eventX"
 */
unsigned char *getDeviceNode (XDeviceInfo devinfo)
{
    Atom  prop;
    Atom act_type;
    int  act_format;
    ...
    if (!prop)
        return NULL;
    if (Success == XIGetProperty(QX11Info::display(), devinfo.id, prop, 0, 1000, False,
                      AnyPropertyType, &act_type, &act_format, &nitems, &bytes_after, &data))
    {
        return data;
    }
    return NULL;
...
GUdevDevice *getGudevDevice(unsigned char* node)
{
...
QString getDeviceProductId(XDevice * device)
{
    int realformat;
    unsigned long nitems, bytes_after;
    unsigned char *data;
    Atom realtype, prop;
    prop = property_from_name("Device Product ID");
    if(!prop) {
        return nullptr;
    }

    if (XGetDeviceProperty (QX11Info::display(), device, prop, 0, 8, False,
                            XA_INTEGER, &realtype, &realformat, &nitems,
                            &bytes_after, &data) == Success) {
        QString str = QString::number(((long*)data)[0]) + QString::number(((long*)data)[1]);
        XFree(data);
        return str;
    }
    return nullptr;
...
bool checkMouseExists()
{
    ...
    QStringList touchPadProductList;
    QStringList mouseProductList;
    for (int i = 0; i < numdevices; i++) {
        XDevice *device = nullptr;
        if (devicelist[i].type == XInternAtom (QX11Info::display(), XI_MOUSE, True) && !strstr(devicelist[i].name, "PS\/2")
                && !strstr(devicelist[i].name, "TrackPoint")) {
            device = XOpenDevice (QX11Info::display(), devicelist[i].id);
        }
        if(device){//mouse
            mouseProductList<<getDeviceProductId(device);
            XCloseDevice (QX11Info::display(), device);
        } else {//touchpad
            device = device_is_touchpad(&devicelist[i]);
            if (device) {
                touchPadProductList<<getDeviceProductId(device);
                XCloseDevice (QX11Info::display(), device);
            }
        }
    ...
    for(QString touchpadProduct : touchPadProductList){
        for(QString mouseProduct : mouseProductList){
            if(touchpadProduct == mouseProduct){
                mouseProductList.removeAll(mouseProduct);
            }
        }
    }
    if(mouseProductList.count()){
        return true;
    } else {
        return false;
    }
}
/**check mouse device end**/
...
void MouseManager::TouchpadCallback (QString keys)
{
    if (keys.compare(QString::fromLocal8Bit(KEY_TOUCHPAD_DISABLE_W_TYPING))==0) {
        ...
        SetNaturalScrollAll (true);                             //设置上移下滚或上移上滚
