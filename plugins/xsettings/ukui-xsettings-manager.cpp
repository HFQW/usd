...
//    USD_LOG(LOG_DEBUG,"key:%s",key);
    ...
    if (g_str_equal (key, SCALING_FACTOR_KEY)) {
        UsdBaseClass::writeUserConfigToLightDM(MODULE_NAME, SCALING_FACTOR_KEY, g_settings_get_double(gsettings, SCALING_FACTOR_KEY));
//        return;
    }
    ...
    if (g_str_equal (key, CURSOR_THEME_KEY)||
        ...
        if (g_str_equal (key, CURSOR_THEME_KEY)) {
            UsdBaseClass::writeUserConfigToLightDM(MODULE_NAME, key, g_settings_get_string(gsettings, key));//GVariant --->QVariant???

        } else {
            UsdBaseClass::writeUserConfigToLightDM(MODULE_NAME, key, g_settings_get_int(gsettings, key));
        }
        return;
    }
